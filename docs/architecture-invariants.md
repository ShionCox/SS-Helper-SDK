...
| SDK owns generic workspace storage; Memory owns every memory rule | `server-plugin/index.js`, `apps/core-extension/src/bridge/internal-bridge.ts`, `packages/sdk/src/contracts/workspace.ts`, Memory `MemoryRepository` and `SdkMemoryHostContext` | The server plugin creates `data/_ss-helper-v0/ss-helper.sqlite3` once per Tavern instance and exposes a single validated internal bridge; retired public workspace/health routes are absent. Memory is a frontend-only consumer of `session.workspace`; SDK contains no Memory schema, worker, dynamic import, route, fact conflict or recall implementation. |
