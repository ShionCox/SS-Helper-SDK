...
| Memory SQLite/schema/server/backup remain Memory-owned | `I:\VUE\SS-Helper-Memory\server`, `manifest.json`, version/server tests; Core `host/tavern-host-port.ts` is transport only | Memory's current post-transcoding baseline is `ab55ec7`. The pre-transcoding historical node `b84d8a1` reports **136 passed** and one environment-gated skip; schema/server/version/fixture integrity remain intact. Core binary-request tests preserve SQLite bytes, content type and authenticated transport without owning data. |
